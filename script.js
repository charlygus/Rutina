...
let wakeLock = null; // Para pantalla encendida
...
// --- NUEVA LÓGICA DE COMPRA POR PASILLOS ---
        ...
        categorias.forEach(cat => {
            if (fila[cat.id]) {
                ...
                items.forEach(rawItem => {
                    ...
                    inventory[cat.id][key].origins.push(`S${fila.Semana} ${fila.Dia}: ${fila.Momento}`);
                });
            }
        });
    });
...
window.abrirReceta = (plato, receta) => { document.getElementById('modal-title').textContent = plato; document.getElementById('modal-body').textContent = receta || "Sin instrucciones."; document.getElementById('recipe-modal').classList.add('open'); activarPantalla(); };
window.cerrarReceta = () => { document.getElementById('recipe-modal').classList.remove('open'); desactivarPantalla(); };
