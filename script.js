...
// --- CARGA DE DATOS (CON PANTALLA DE CARGA) ---
async function loadData() {
    ...
    try {
        ...
        label.textContent = "Error"; console.error(e);
    } finally {
        // 🟢 OCULTAR PANTALLA DE CARGA CUANDO TERMINA (BIEN O MAL)
        setTimeout(() => {
            document.getElementById('loading-screen').classList.add('hidden');
        }, 500); // Un pequeño retardo para que sea suave
    }
}
...
function renderWeek() {
    ...
    if (filasSemana.length === 0) { container.innerHTML = `<p style="text-align:center; padding:20px; color:#999;">Vacío...</p>`; return; }
    ...
    filasSemana.forEach(fila => {
        ...
        if(diaLimpio === "Miercoles") diaKey = "Miércoles"; if(diaLimpio === "Sabado") diaKey = "Sábado";
        ...
        if (platos) {
            ...
            container.innerHTML += `
                <div class="day-item ${claseExtra}" ${idDia}>
                    <div class="day-header" style="display:flex; justify-content:space-between;"><span>${nombreDia}</span><span style="color:#888; font-weight:normal;">${formatDate(fechaTarjeta)}</span></div>
                    <div class="day-body">${renderMealRow('Desayuno', platos.Desayuno)}${renderMealRow('Comida', platos.Comida)}${renderMealRow('Cena', platos.Cena)}</div>
                </div>`;
        }
    });
...
// --- FUNCIONES EXTRA (Recetas, Compra, Peso) ---
window.abrirReceta = (plato, receta) => { document.getElementById('modal-title').textContent = plato; document.getElementById('modal-body').textContent = receta || "Sin instrucciones."; document.getElementById('recipe-modal').classList.add('open'); };
...
function renderShopping() {
    const list = document.getElementById('shopping-list'); const subtitle = document.getElementById('shopping-subtitle'); list.innerHTML = '';
    ...
    const inventory = {}; const colsIngredientes = ['Carniceria', 'Pescaderia', 'Fruteria', 'Refrigerados', 'Despensa'];
    const filasObjetivo = planificadorData.filter(r => targetWeeks.some(w => r.Semana == w));
    filasObjetivo.forEach(fila => { colsIngredientes.forEach(col => { if (fila[col]) { const items = fila[col].toString().split(','); items.forEach(rawItem => { let item = rawItem.trim(); if (!item) return; let key = item.toLowerCase(); let display = item.charAt(0).toUpperCase() + item.slice(1); if (!inventory[key]) inventory[key] = { name: display, count: 0, origins: [] }; inventory[key].count++; inventory[key].origins.push(`S${fila.Semana} ${fila.Dia}: ${fila.Momento}`); }); } }); });
    const sortedKeys = Object.keys(inventory).sort(); if (sortedKeys.length === 0) { list.innerHTML = '<li style="color:#aaa; padding:15px;">No hay ingredientes.</li>'; return; }
    sortedKeys.forEach(key => { const data = inventory[key]; const id = `shop-${supermarketMode?'super':'w'}-${targetWeeks.join('')}-${key.replace(/\s+/g, '')}`; const checked = localStorage.getItem(id) === 'true'; const detailsHtml = data.origins.map(o => `<span>• ${o}</span>`).join('<br>'); list.innerHTML += `<li><input type="checkbox" id="${id}" ${checked ? 'checked' : ''} onchange="saveStatus('${id}', this.checked)"><div class="item-content"><label class="item-title" for="${id}">${data.name} <span style="font-weight:normal; color:#555;">(x${data.count})</span></label><small class="item-details">${detailsHtml}</small></div></li>`; });
}
...
function copiarLista() { const items = document.querySelectorAll('#shopping-list li'); let texto = "🛒 *LISTA DE LA COMPRA*\n\n"; items.forEach(li => { const nombre = li.querySelector('.item-title').innerText.split('(')[0].trim(); const cantidad = li.querySelector('.item-title span').innerText; const check = li.querySelector('input').checked ? "✅" : "⬜"; texto += `${check} ${nombre} ${cantidad}\n`; }); navigator.clipboard.writeText(texto).then(() => { alert("¡Copiada al portapapeles!"); }); }
function limpiarChecks() { if(confirm("¿Borrar marcados?")) { Object.keys(localStorage).forEach(key => { if(key.startsWith('shop-')) localStorage.removeItem(key); }); renderShopping(); } }
window.saveStatus = (id, state) => { localStorage.setItem(id, state); };
window.showTab = (name) => { document.querySelectorAll('.tab-content').forEach(c => c.classList.remove('active')); document.querySelectorAll('.tab-link').forEach(b => b.classList.remove('active')); document.getElementById(name + '-view').classList.add('active'); if (event) event.currentTarget.classList.add('active'); if (name === 'weight') cargarHistorialPeso(); };
